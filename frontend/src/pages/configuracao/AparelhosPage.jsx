        ...
        <CardContent className="p-5 space-y-4">
          ...
          <div className="block md:hidden">
            {isLoading ? <Spinner /> : !data.length ? (
              <p className="text-sm text-muted-foreground text-center py-8">Nenhum aparelho cadastrado.</p>
            ) : (
              <div className="space-y-2">
                {data.map(r => (
                  <div key={r.apar_id} className="rounded-lg border border-border bg-fluir-dark-2 p-3">
                    <div className="flex items-start justify-between gap-2">
                      <p className="font-medium text-sm">{r.apar_nome}</p>
                      <div className="flex items-center gap-1 flex-shrink-0">
                        <Badge variant={MODALIDADE_VARIANT[r.apar_modalidade] || 'default'}>
                          {MODALIDADES.find(m => m.value === r.apar_modalidade)?.label || r.apar_modalidade}
                        </Badge>
                        <Badge variant={r.apar_ativo ? 'success' : 'secondary'}>{r.apar_ativo ? 'Ativo' : 'Inativo'}</Badge>
                      </div>
                    </div>
