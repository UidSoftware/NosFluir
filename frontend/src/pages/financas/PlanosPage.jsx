          ...
          <div className="block md:hidden">
            {isLoading ? (
              ...
              <div className="space-y-2">
                {data.map(r => (
                  <div key={r.plan_id} className="rounded-lg border border-border bg-fluir-dark-2 p-3 space-y-1.5">
                    <div className="flex items-start justify-between gap-2">
                      <p className="font-medium text-sm">{r.serv_nome || '—'}</p>
                      <Badge variant="secondary">{TIPO_LABEL[r.plan_tipo_plano] || r.plan_tipo_plano}</Badge>
                    </div>
                    <div className="flex items-center justify-between mt-2">
                      <div className="flex flex-col gap-0.5">
                        <span className="text-sm font-semibold text-fluir-cyan">{formatCurrency(r.plan_valor_plano)}</span>
