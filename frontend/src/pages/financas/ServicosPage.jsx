          ...
          <div className="block md:hidden">
            {isLoading ? (
              ...
              <div className="space-y-2">
                {data.map(r => (
                  <div key={r.serv_id} className="rounded-lg border border-border bg-fluir-dark-2 p-3 space-y-1.5">
                    <div className="flex items-start justify-between gap-2">
                      <p className="font-medium text-sm">{r.serv_nome}</p>
                      <BooleanBadge value={r.serv_ativo} />
                    </div>
                    {r.serv_descricao && (
                      <p className="text-xs text-muted-foreground">{r.serv_descricao}</p>
                    )}
                    <div className="flex items-center justify-between mt-2">
                      <span className="text-sm font-semibold text-fluir-cyan">{formatCurrency(r.serv_valor_base)}</span>
