...
import { ListTodo, Trash2, Clock, Calendar, Sparkles, Plus, UserPlus, X } from 'lucide-react'
...
function AulaExperimentalForm({ agendamento, aulaExistente, onClose }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const msg = d ? (typeof d === 'string' ? d : Object.entries(d).map(([k,v]) => `${k}: ${Array.isArray(v)?v.join(', '):v}`).join(' | ')) : 'Erro.'
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div>
          ...
          <div className="space-y-3">
            ...
            <FormField label="Equilíbrio"><TA name="aexp_equilibrio" placeholder="Uma perna + elevação lateral — estabilidade..." /></FormField>
            <FormField label="Coordenação"><TA name="aexp_coordenacao" placeholder="Perdigueiro invertido — execução..." /></FormField>
    ...
    <Dialog open onOpenChange={v => { if (!v) onClose() }}>
      <DialogContent className="max-w-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3 py-2">
          <p className="text-xs text-muted-foreground">Dados pré-preenchidos do agendamento. Preencha o CPF antes de salvar.</p>
    ...
    <Card className="border-fluir-purple/30">
      <CardContent className="pt-4 space-y-4">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {formatDate(row.age_data_agendada)} {row.age_hora_agendada?.slice(0,5)} · {row.age_telefone}
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-sm">
          ...
          {row.age_observacoes && <div className="col-span-2"><span className="text-xs text-muted-foreground">Obs: </span><span className="text-xs">{row.age_observacoes}</span></div>}
        </div>
...
export default function AgendamentosPage() {
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="Agendamentos"
        description="Solicitações do site e agendamentos de aulas experimentais"
      />
